  ...
  return (
    <div className="relative">
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
        className={`flex items-center space-x-3 px-6 py-4 rounded-xl border transition-all duration-200 ${
          isOpen 
            ? 'bg-gray-700/50 border-cyan-500 ring-2 ring-cyan-500/20' 
            : 'bg-gray-800/50 border-gray-700 hover:border-gray-600'
        }`}
      >
        <div className={`flex items-center space-x-3 ${currentPriority.textColor}`}>
          <IconComponent className="w-5 h-5" />
          <span className="text-base font-medium text-white">{currentPriority.label}</span>
        </div>
        <ChevronDownIcon 
